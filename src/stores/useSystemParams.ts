...
function getSystemParamsByKeys(data: { classKeyList: string[] }) {
    return Http.CrudRequest<SystemParamsItem[][]>({
...
export const useSystemParamsStore = defineStore(
    'systemParams',
    () => {
        /**
         * 缓存的到期时间，单位为毫秒（8小时）。
         ...
         */
        function _isCacheValid(cacheKey: string, cacheKeys: Record<string, ResultType[]>, cacheKeysExpTime: Record<string, number>): boolean {
        ...
        async function GetSystemParams<T>(mapList: Map<KeysOf<T>, MapValueType>): Promise<Map<KeysOf<T>, ResultType[]>> {
            ...
            mapList.forEach((v, k) => {
                ...
                if (_isCacheValid(cacheKey, cacheKeys, cacheKeysExpTime))
            ...
            if (noCacheKeys.length > 0) {
                ...
                data.forEach((items) => {
                    if (items.length > 0) {
                        const idx = requestKeys.indexOf(items[0].classKey)
                        const MapKey = columnsKeys[idx]
                        const Param = mapList.get(MapKey)

                        if (Param) {
                            const cacheKey = _getHashKey(MapKey, Param)
